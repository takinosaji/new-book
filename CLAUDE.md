...
- **Word document** (`_book/<output-file>-<version>.docx`) — generated from the book configuration

## Version

`book.subtitle` in `src/_quarto.yml` (`"Version X.Y.Z"`) is the single source of truth.
`render_book.py` reads it via `book_config.py` and names the docx `<output-file>-<version>.docx`
(falling back to `<output-file>.docx` if the subtitle has no version). Bump the version there and nowhere else.
