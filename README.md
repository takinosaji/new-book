...
## 5. Adapting This Template

This repository is a starting point. Adapting a fresh fork means renaming the book title, package
name, author, and URLs, and replacing the placeholder introduction.

**The canonical workflow is the `/x-init-fork` Claude Code skill.** Run it in a fresh session after
cloning — it asks a few questions and performs all of the above automatically. The full step-by-step
specification (also usable as a manual checklist without Claude Code) lives in
[`.claude/skills/x-init-fork/SKILL.md`](.claude/skills/x-init-fork/SKILL.md).
